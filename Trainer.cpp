...
Trainer :: Trainer(string in_name,int in_id,char in_code,double in_speed, Point2D in_loc) : GameObject(in_loc,in_id,in_code){
    speed = in_speed;
    name = in_name;
    state = STOPPED;
    cout << "Trainer constructed" << endl;
...
void Trainer :: StartBattling(unsigned int num_battles)
{
    ...
    else
    {
        ...
        health=health-followedhealth(isfollowed());
        experience=experience-(battles_to_buy*(current_gym->GetExperiencePerBattle()));
...
void Trainer :: ShowStatus()
{
    ...
    cout<<"Being followed by "<<NumberPokemon<<" Wild pokemon"<<endl;
}
...
static double GetRandomAmountOfPokeDollars(){
    //Creates a random amount of money from 1-5, these values could be changed
    double min = 1.0;
    double max = 5.0;
...
//checks to see if trainer is followed by wild pokemon
bool Trainer:: isfollowed(){
    if (NumberPokemon>0){
        return true;
    }else{
        return false;
...
int Trainer::followedhealth(bool following){
    if (following==true){
        int count= NumberPokemon*1;
        return count;
    }else{
        return 0;
    }
...
unsigned int Trainer :: getnumwildpokemons(){
    return NumberPokemon;
}

void Trainer:: wildNum(bool wild){
    if (wild==true){
